...
- The tracker runs in a dedicated browser worker on accepted synchronized frames and persists `refinement/tracking.json`. Pose-independent RANSAC verification, inlier-only scoring, and bounded component repair connect 105/105 seesaw frames and 145/145 second-capture frames in replay.
...
- Offline replay of the phase split preserves 145/145 connectivity and eight loops while reducing capture-phase work to approximately 41 ms/frame on desktop. `refinement/tracking.json` now records capture-phase time, maximum frame time, deferred time, and retained grayscale bytes for phone validation.
...
- The second capture yields eight long-range loop closures that validate against independent COLMAP poses; the seesaw replay still yields none. Global SE(3) correction is not yet implemented, so direct-train readiness remains disabled and export continues to request downstream SfM.
