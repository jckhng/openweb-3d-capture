...
Current scope: capture preflight, stop-and-shoot object coverage guidance, and safe handoff to Spirula Studio and LichtFeld Studio. The Android Chrome WebXR recorder evaluates up to four synchronized candidates per second, locks the centered subject in world space at Start, projects that target into every camera frame, and rejects off-target framing, subject-region blur, excessive motion, close-range fixed-focus failures, bad tracking, and unsynchronized images. After movement it waits through a three-candidate settling window and records one reconstruction image. Each globe cell requires two object-relative viewpoints separated by at least 6°, preventing a stationary burst from masquerading as viewpoint diversity. WebXR poses support navigation and diagnostics; production reconstruction uses downstream visual SfM.
...
7. Stop and hold through the settling prompt. The app saves one sharp representative as **VIEWPOINT 1 / 2**.
8. Take a small sideways step within the same sector, stop again, and wait for the orange **SECTOR CAPTURED** confirmation. The two viewpoints must be separated by at least 6° around the object.
9. Repeat for the required standard, raised, top, and low checkpoints. Select **Finish scan** when the globe and readiness result show sufficient coverage.
10. Export the capture for Spirula, LichtFeld, or as a full archive.
11. Inspect `capture.json`, `transforms.json`, `telemetry/frames.jsonl`, `debug/session.jsonl`, sampled `debug/rejected/` quality crops, images, optional depth, and IMU samples.
12. Reconstruct the images through Spirula native SfM or LichtFeld's COLMAP Reconstruction plugin before training. Do not treat the WebXR transform file as final reconstruction poses.
...
During capture, a translucent 12-longitude by 4-layer globe rotates continuously with the live WebXR azimuth and elevation. Uncaptured cells are light blue, the active cell is blue, and completed cells are orange; reverse-side cells remain visible. On devices that grant CPU depth, accepted viewpoints also build a local depth constellation inside the globe: blue points are newly observed surface voxels and orange points were supported by multiple accepted viewpoints. This display-only capture map is real measured geometry, is never uploaded, and does not affect readiness or the exported seed cloud. The 25 required checkpoints comprise six low cells (-20° to 5°), twelve standard handheld cells (5° to 35° above the object), six raised cells (35° to 60°), and one top cap (60° to 90°). This makes the normal phone-at-chest/head-height, camera-pointed-slightly-downward posture the dense ring. Coverage is derived from camera position around the estimated object, not phone pitch alone. A cell lights only after two sharp, low-motion viewpoints at least 6° apart are retained.
...
Both destination packages preserve WebXR poses under `open3dcapture/telemetry/frames.jsonl` but declare downstream SfM as the final pose authority. Near-duplicate camera positions collapse into viewpoint groups. Each globe cell retains at most four representatives separated by at least 6°; destination exports reject motion scores above 0.4 and rank representatives with the attributed hybrid sharpness metric. A complete 25-cell capture therefore begins at 50 distinct viewpoints and is capped at 100 destination images. Incomplete but sufficiently distributed captures use the same bounded sector selection; very sparse captures fall back to every image. Exporting a non-ready capture produces both an interactive warning and a warning in the handoff README. The choice and selected frame IDs are recorded in `open3dcapture/export.json`.
